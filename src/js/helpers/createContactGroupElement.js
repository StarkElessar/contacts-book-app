export const createContactGroupElement = () => {
  const contactGroupElement = document.createElement('li')
  contactGroupElement.classList.add('contacts-group__item')
  contactGroupElement.innerHTML = `
  ...
  `
  contactGroupElement.querySelector('input').oninput = ({ target }) => target.setAttribute('value', target.value)
  contactGroupElement.querySelector('button').onclick = ({ currentTarget }) => currentTarget.parentElement.remove()
